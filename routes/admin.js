...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (!isPdfExt || file.mimetype !== "application/pdf") {
...
router.post(
  ...
  async (req, res) => {
    ...
    // Magic-byte verification BEFORE uploading to Backblaze B2:
    // 1. Direct header signature check: PDF files start with '%PDF-' (hex: 25 50 44 46 2d)
    const hasPdfMagic =
      buf.length >= 5 &&
      buf[0] === 0x25 &&
      buf[1] === 0x50 &&
    ...
    // 2. Comprehensive deep inspection using file-type
    let detected = null;
    try {
    ...
    if (!hasPdfMagic || !detected || detected.mime !== "application/pdf") {
      return res.status(400).json({
        error: "Invalid file content: Not a valid PDF document (magic-byte check failed)",
      });
    }

    // Now safely upload to Backblaze B2 knowing the file is 100% verified authentic PDF
    const bucket = (process.env.B2_BUCKET || "").trim();
    const key = `pdfs/${Date.now()}-${crypto.randomBytes(6).toString("hex")}.pdf`;
